...
def main():
    ...
    if not os.path.exists(train_dir):
        ...
        exit(1)
    dataset_train = MyDataset(train_dir,"/ssd3/sqy/ImageNet/train_label.txt")
    ...
    if not os.path.isdir(eval_dir):
        ...
        if not os.path.isdir(eval_dir):
            ...
            exit(1)
    dataset_eval = MyDataset(eval_dir,"/ssd3/sqy/ImageNet/validation_label.txt")
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        #if args.local_rank == 0:
        #    _logger.info('target: {}'.format(target[target!=target.max()]))
        import numpy as np
        if batch_idx==1:
            result = target.cpu()
            result1 = np.array(result)
            
            np.savetxt('target.txt',result1)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='',writer=None):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
